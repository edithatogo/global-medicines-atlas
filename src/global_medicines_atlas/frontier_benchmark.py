...
Candidate = Literal[
    "python_fallback", "duckdb", "polars", "arrow", "xet_restore"
]
...
def benchmark_fixture(
    ...
    observations = [
        BenchmarkObservation(
            candidate="python_fallback",
            status="measured",
            rows_scanned=len(normalized),
            rows_returned=len(result),
            operations=len(normalized) + len(selected),
            output_sha256=workload.output_sha256,
            fallback="portable_python",
...
def _optional_engine_observations(
    ...
    try:
        ...
        output = (
            frame
            .filter(polars.col("active") == True)  # ruff: ignore[true-false-comparison]
            .select(["id", "value"])
            .sort(["id", "value"])
            .to_dicts()
        )
        ...
        observations.append(
            _unavailable(
                "polars", f"optional engine unavailable: {type(exc).__name__}"
            )
        )
    except (
        Exception
    ) as exc:  # pragma: no cover - platform-specific engine failures
        observations.append(
            _unavailable(
                "polars", f"optional engine failed closed: {type(exc).__name__}"
            )
        )
    ...
    try:  # ruff: ignore[too-many-statements-in-try-clause]
        ...
        filtered = table.filter(
            compute.equal(table["active"], arrow.scalar(value=True))
        )
        output = [
        ...
        observations.append(
            _unavailable(
                "arrow", f"optional engine unavailable: {type(exc).__name__}"
            )
        )
    except (
        Exception
        ...
        observations.append(
            _unavailable(
                "arrow", f"optional engine failed closed: {type(exc).__name__}"
            )
        )
    ...
    try:  # ruff: ignore[too-many-statements-in-try-clause]
        ...
        observations.append(
            _unavailable(
                "duckdb", f"optional engine unavailable: {type(exc).__name__}"
            )
        )
    except (
        Exception
        ...
        observations.append(
            _unavailable(
                "duckdb", f"optional engine failed closed: {type(exc).__name__}"
            )
        )
    return observations
...
def _measured(
    ...
    if digest != expected_output:
        return _unavailable(
            candidate,
            "optional engine parity mismatch; retained as unavailable",
        )
    return BenchmarkObservation(
        candidate=candidate,
        status="measured",
        ...
        operations=row_count + len(output),
