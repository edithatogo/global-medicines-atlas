...
import hashlib
import json
import subprocess  # ruff: ignore[suspicious-subprocess-import] - fixed interpreter
import sys
...
OUTPUT = ROOT / "research/preregistration/submission"
...
def test_preregistration_contract_is_strict_and_offline_only() -> None:
    ...
    assert package["outcome_dimensions"] == [
        "regulatory_status",
        "funding_status",
    ]
...
def test_governance_statements_and_append_only_registers_are_present() -> None:
    ...
    assert (
        package["data_management"]["restricted_payload_redistribution"] is False
    )
...
def test_bundle_is_deterministic_and_manifest_checks_every_attachment(
    tmp_path: Path,
) -> None:
    ...
    assert {
        p.relative_to(first): p.read_bytes()
        for p in first.rglob("*")
        if p.is_file()
    } == {
        p.relative_to(second): p.read_bytes()
...
def test_citations_are_resolvable_and_have_stable_identifiers() -> None:
    ...
    assert all(
        item.get("doi") or item.get("url") for item in citations["citations"]
    )
...
def test_documented_offline_commands_run_in_clean_output_directory(
    tmp_path: Path,
) -> None:
    ...
    subprocess.run(  # ruff: ignore[subprocess-without-shell-equals-true]
        [
            sys.executable,
            "-m",
            "scripts.build_academic_preregistration",
            "--output",
            str(output),
        ],
        cwd=ROOT,
    ...
    subprocess.run(  # ruff: ignore[subprocess-without-shell-equals-true]
        [
            sys.executable,
            "-m",
            "scripts.validate_academic_preregistration",
            "--bundle",
            str(output),
        ],
        cwd=ROOT,
