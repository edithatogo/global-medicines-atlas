...
def test_lineage_receipt_is_sorted_deterministic_and_payload_free() -> None:
    receipt = build_research_lineage_receipt(
        export_id="export-1",
        revision="a" * 40,
        artifacts=(_artifact("output.json", "output"), _artifact("input.json", "input")),
    )
    assert receipt.payloads_embedded is False
    assert [item.identifier for item in receipt.artifacts] == ["input.json", "output.json"]
...
def test_lineage_receipt_requires_both_roles_and_unique_ids() -> None:
    ...
    with pytest.raises(ValueError, match="identifiers must be unique"):
        build_research_lineage_receipt(
            export_id="export-1",
            revision="a" * 40,
            artifacts=(_artifact("same.json", "input"), _artifact("same.json", "output")),
        )
