...
JSON = b'[{"native_id":"A-1","quantity":7,"listed":true}]'
...
def test_binary_payload_emits_manifest_without_replacement_decoding(
    ...
    assert outcome.acquisition_manifest_path.name == "acquisition_manifest.parquet"
    ...
    assert manifest.column("payload_sha256")[0].as_py() == receipt.payload.sha256
...
def test_adapter_records_preserve_native_schema_and_have_independent_evidence(
    ...
    assert outcome.source_records_transformation_run.output.sha256 == sha256(
    ...
    projected = {
        item["namespace"] for item in lineage["outputs"]
    }
