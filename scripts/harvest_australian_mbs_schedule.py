...
def fetch_url_bytes(url: str) -> tuple[bytes, str]:
    return fetch_url_bytes_governed(
        url, allowed_domains=ALLOWED_MBS_DOMAINS, user_agent=USER_AGENT
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Harvest ongoing Australian MBS schedule releases"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Discover releases without staging",
    )
    parser.add_argument(
        "--stage-dir", type=Path, default=Path("build/harvest/mbs_schedule")
    )
    parser.add_argument(
        "--publish-hosted",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="Max monthly releases to probe",
    )
    parser.add_argument(
        "--receipt-output",
        type=Path,
        default=Path("build/mbs-schedule-harvest-receipt.json"),
    )
