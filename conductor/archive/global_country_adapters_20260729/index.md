# Global Country Adapters Track

Status: archived after PR #55 merged; external live qualification continues in
GitHub issue #54.

- [GitHub parent issue #7](https://github.com/edithatogo/global-medicines-atlas/issues/7)
- [Phase 1 issue #8](https://github.com/edithatogo/global-medicines-atlas/issues/8) — registry, catalog, and first cohort
- [Phase 2 issue #9](https://github.com/edithatogo/global-medicines-atlas/issues/9) — receipt-backed ingestors
- [Phase 3 issue #10](https://github.com/edithatogo/global-medicines-atlas/issues/10) — comparison and coverage
- [Phase 4 issue #11](https://github.com/edithatogo/global-medicines-atlas/issues/11) — global source census
- [GitHub Project #35](https://github.com/users/edithatogo/projects/35)
- [Specification](./spec.md)
- [Implementation plan](./plan.md)
- [Metadata](./metadata.json)
- [Evidence ledger](./evidence.jsonl)

This track owns the reusable jurisdiction registry and phased onboarding beyond
the New Zealand migration boundary.
